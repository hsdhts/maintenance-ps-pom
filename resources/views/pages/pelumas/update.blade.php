...
<form action="/pelumas/update/" method="post">
    ...
    <div class="container-lg mt-5">
        ...
        <input type="hidden" name="id_old" value="{{ $pelumas->id }}">

        <div class="mb-4">
            <label for="pic" class="form-label">Nama Sparepart</label>
            <select class="form-select @error('sparepart_id') is-invalid @enderror" id="sparepart_id" aria-label="Pilihan untuk Sparepart" name="sparepart_id">
                @foreach ($sparepart as $u)
                <option value="{{ $u->id }}" {{ $u->id == $pelumas->sparepart_id ? 'selected' : '' }}>{{ $u->id }}-{{ $u->nama_sparepart }}</option>
                @endforeach
            </select>
            @error('sparepart_id')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('metode_pelumasan') is-invalid @enderror" id="metode_pelumasan" placeholder="Metode Pelumasan" value="{{ $pelumas->metode_pelumasan }}" name="metode_pelumasan">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('lubricant') is-invalid @enderror" id="lubricant" placeholder="Lubricant" value="{{ $pelumas->lubricant }}" name="lubricant">
            @error('lubricant')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="input-group mb-5">
            ...
            <input type="text" class="form-control bagian_form @error('lubricating_interval') is-invalid @enderror" aria-label="lubricating interval" name="lubricating_interval" value="{{ $pelumas->lubricating_interval }}" id="lubricating_interval" readonly>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('pelumasan_terakhir') is-invalid @enderror" id="sparepart" placeholder="Nama Sparepart" value="{{ $pelumas->pelumasan_terakhir }}" name="pelumasan_terakhir">
